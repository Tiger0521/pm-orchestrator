...
当 handoff 含 `sourceProduct` 时，这是从产品库直启的拆解项目：先读取该产品的需求卡片、设计文档、能力文档和已有 Story 作为只读上游，再按本阶段 draft/persist/validate 门禁执行。不得要求本地 Epic/Feature、不得复制产品库文档、不得修改来源产品的已有文档；**新建 Story 与常规项目一致落盘产品库来源产品目录**（persist 轨道写入 `<能力路径>/用户故事/`），溯源矩阵与常规项目一致留在过程项目，不以过程项目作为 Story 的中间落点，并在回执和正式产物中保留来源产品 ID 与来源文档路径。
