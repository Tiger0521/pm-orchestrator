...
文件路径：`<产品目录>/<能力路径>/用户故事/网资-能力-故事标题故事.md`。每条 Story 必须写入其唯一归属 Feature 对应能力路径下的 `用户故事/` 子目录；不得写入过程项目 `docs/` 或其他能力的目录。
...
1. 落盘前扫描产品库中当前 Feature 的故事文档（`<产品目录>/<能力路径>/用户故事/*.md`）的 frontmatter ID。
