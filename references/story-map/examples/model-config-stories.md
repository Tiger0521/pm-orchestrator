...
本示例展示 story-map 阶段的完整产出质量标杆。包含两条 Story（创建模型配置 + 查看模型配置列表）和一份溯源矩阵，每条 Story 含完整的 GWT 验收标准、`journey_stage` 与 `requirementEntryId` 关联，均采用加粗关键词领条格式。

---

## 价值主张澄清示例

价值主张含模糊词时，审阅的是"对用户意味着什么可感知结果"，不是性能阈值。以下展示按 Feature 分组批量审阅中，某条 Story 说不清时的一次条件聚焦提问。

【当前故事 · story-001（草稿）】
作为 **算法工程师**，我想要 **创建模型配置**，以便于 __「即时生效」——说不清对用户意味着什么可感知结果__。

验收标准（草稿）：
1. **成功创建**：Given 算法工程师已登录，When 填写名称/版本/参数并提交，Then 系统创建配置并返回"创建成功"
2. __名称重复时如何处理？场景未明__

---
本轮提问：故事里「即时生效」这个价值，对算法工程师意味着什么可感知的结果？

A. **无需重启服务**即可让新配置生效（推荐）：上游 Feature 已声明配置热加载能力，"即时"对用户的最直接感知是不重启
B. **无需修改代码**即可新增配置类型：侧重扩展性而非生效方式
C. 补充描述
D. 强制跳过

说明：本例问的是价值澄清（用户可感知结果），**不是**"延迟阈值多少秒""用 push 还是 pull"——后者属详细设计范畴，不在 Story 层审阅。默认按 Feature 分组整组审阅，仅当某条 Story 的三块内容说不清时才这样聚焦提问。
...
|----|-----------|------|--------|-------------|----------|--------------|
| story-001 | 创建模型配置 | 算法工程师 | P0 | 3 | 建址-审核核准 | 网资-REQ-001 |
| story-002 | 查看模型配置列表 | 算法工程师 | P0 | 2 | 建址-录入导入 | 网资-REQ-002 |
...
- [x] 每条 Story 的旅程阶段都属于已验证旅程叙事线节点
- [x] 每条 Story 的需求台账条目在台账中实际存在