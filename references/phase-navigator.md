...
本文件是 pm-orchestrator 的全局阶段地图与状态检测规范。主调度器在用户无明确阶段意图时读取本文件展示进度，展示前先按 `SKILL.md`「用户引导与安抚（开场协议）」完成介绍与安抚，再按本文件输出入口引导（模板见下）；每次 subagent 委派前依据本文件判定「当前阶段 + 进度」并注入 `navigationContext`；委派返回后按阶段产出更新进度。
...
无明确阶段意图时，输出按「介绍 → 状态 → 菜单 → 安抚」组织。有进行中项目时带进度地图；首次进入、无项目时先给能力介绍。两套模板：
...
- 继续当前阶段
- 开始一个新想法
- 单独做一件事（更新架构设计文档 / 修补能力分类 / 修改已有原型）

不用记流程——告诉我想做什么就行，我会一步步带你走；随时可以问我「现在到哪了」。
```

首次进入、无进行中项目时：

```
我可以帮你完成产品设计全流程：需求分析 → 用户故事与故事地图 → 详细设计 → 迭代计划；也可以单独做这些事：更新架构设计文档的产品矩阵、修补产品库能力分类、修改已有原型。

当前还没有进行中的项目。你可以说出一个新想法，或选择继续以前的项目——流程细节你不需要记，每次告诉我下一步想做什么即可。
```
