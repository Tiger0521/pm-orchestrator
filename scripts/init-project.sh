...
if [ -n "$selected_product_library_path" ]; then
  ...
  # 检查产品库中是否已存在同名产品目录
  if [ -n "$matched_product_id" ] && [ -d "$selected_product_library_path/$matched_product_id" ]; then
    echo "WARNING: product with same name already exists in library: $matched_product_id" >&2
