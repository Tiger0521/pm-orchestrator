...
以下路径均相对 `skillPath` 解析。合并后的 story-map 阶段参考面是 `references/story-map/` 下的**全部文件**，均为本轮或后续轮次必须读取的强制门禁（含 `grilling-protocol.md`、`confirmation-method.md`、`examples/` 等拆解/确认相关文件）；`mode` 只决定读取顺序（哪些先读、哪些等动作触发后再读），不豁免任何文件：
