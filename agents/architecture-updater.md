...
1. **必读**：`references/product-library/contract.md`（产品矩阵约定）与 `productArchitectureDesignPath` 当前版本；`productArchitectureDesignPath` 文件缺失或产品矩阵标记缺失时返回 `blocked`。
...
5. **验证与汇报**：写入后运行 `validate-product-library-lite.sh`（产品库身份校验）确认根文档完好，再汇报每个产品的新增/移除统计。
