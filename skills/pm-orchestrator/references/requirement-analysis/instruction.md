...
诊断报告默认只作为 `draft` 交互产物，不写入正式 `docs/`。用户确认后，将诊断摘要写入 `phase-summary.md`，方案选择写入 `decision-log.md`，待验证事项写入 `tracking-log.md`；仅当用户明确要求保留完整诊断报告时，才写入 `docs/requirement-analysis/diagnostic-report-001.md`。
...
- 产出 Epic，包含需求背景、产品定位、端到端业务闭环、目标用户、范围边界、建设思路、成功指标和主要风险。
- 产出 Feature，包含需求背景、能力目标、用户角色、业务场景、前后对比、业务流程、输入输出数据、业务规则、异常分支、技术可行性、资源投入和验收标准；多个问题同属一个产品闭环时，可以产出多个 Feature 或一个 Feature 下的多个业务场景。
...
### 需求卡片：`docs/requirement-analysis/req-001.md`
...
### Epic：`docs/requirement-analysis/epic-001.md`
...
### Feature：`docs/requirement-analysis/feature-001.md`
